...
// Alert Pro — the monetization surface (C-111 §7: priority alerts + custom
// rules). A real Pi User-to-App payment; also satisfies the Pi Portal "Process a
// Transaction" step. ADR-007 dual-mode: Hub navigation → Mode 1 (Hub modal);
// standalone → Mode 2 (createU2APayment). Approves under PI_API_KEY_ALERT
...
const ITEM_ID = 'alert_pro_monthly';
const MEMO    = 'TEC Alert — Pro (1 month)';
...
export function AlertPro() {
  ...
  const handleUpgrade = async () => {
    ...
    try {
      ...
      setStatus('paying');
      const result = await createU2APayment(PRICE, MEMO, { item_id: ITEM_ID, plan: 'alert_pro' }, internalId);
  ...
  if (status === 'success') {
    return (
      <div style={{ ...card, borderColor: `${TEC_COLORS.success}66` }}>
        <div style={{ fontSize: 15, fontWeight: 800, color: TEC_COLORS.success }}>✅ Alert Pro active</div>
        <div style={{ fontSize: 12, color: TEC_COLORS.subtext, marginTop: 6 }}>
          Payment received. Priority alerts + custom rules unlocked 🔔.
        </div>
      </div>
    );
  }
  ...
  return (
    <div style={card}>
      <div style={{ display: 'flex', alignItems: 'baseline', justifyContent: 'space-between', gap: 12 }}>
        <div style={{ fontSize: 15, fontWeight: 800, color: TEC_COLORS.gold }}>🔔 Alert Pro</div>
      ...
      <div style={{ fontSize: 12, color: TEC_COLORS.subtext, marginTop: 8, lineHeight: 1.5 }}>
        ...
        Pi news). Alert presents + routes; the owning app still owns resolution.
      </div>
