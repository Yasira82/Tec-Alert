...
import { Icon, type AlertIconName } from './Icon';
import { useTranslation } from '@/lib/i18n';

export type AlertTab = 'inbox' | 'watchlist' | 'pro' | 'settings';

export function BottomNav({ active, onSelect }: { active: AlertTab; onSelect: (t: AlertTab) => void }) {
  const { t } = useTranslation();
  const ITEMS: { key: AlertTab; icon: AlertIconName; label: string }[] = [
    { key: 'inbox',     icon: 'bell',     label: t.alert.nav.inbox     },
    { key: 'watchlist', icon: 'bookmark', label: t.alert.nav.watchlist },
    { key: 'pro',       icon: 'star',     label: t.alert.nav.pro       },
    { key: 'settings',  icon: 'settings', label: t.alert.nav.settings  },
  ];
  return (
    <nav style={{
