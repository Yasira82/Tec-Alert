...
const APP_URL    = httpOr(process.env.NEXT_PUBLIC_APP_URL, 'https://alert.tecosystem.app');
const APP_NAME   = process.env.NEXT_PUBLIC_APP_NAME   ?? 'TEC Alert';
    ...
    <div style={{
      ...
      <div style={{ textAlign: 'center' }}>
        ...
        <div style={{ fontSize: 13, color: TEC_COLORS.subtext, marginBottom: 6 }}>
          TEC ECOSYSTEM · ALERTS
        </div>
        <div style={{ fontSize: 13, color: TEC_COLORS.subtext, marginBottom: 32, maxWidth: 320, lineHeight: 1.5 }}>
          One smart inbox for everything that matters — your TEC activity and the
          Pi community, in one place.
        </div>
        <button
          onClick={handleLogin}
          disabled={isLoading}
