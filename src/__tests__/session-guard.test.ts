...
const visit = (cookies: Record<string, string>, path = '/app?q=1') => {
  const req = new NextRequest(`https://alert.tecosystem.app${path}`);
...
describe('the page guard admits a WHOLE session only', () => {
  ...
  it('goes to the Hub SSO, keeping path AND query, and marks the round trip', () => {
    ...
    expect(back.toString()).toBe('https://alert.tecosystem.app/app?q=1&__sso=1');
  });
...
const askMe = async (cookies: Record<string, string>) => {
  const req = new NextRequest('https://alert.tecosystem.app/api/auth/me');
  for (const [k, v] of Object.entries(cookies)) req.cookies.set(k, v);
